...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("reversalcoin"))
        return false;
...
{
    // Convert reversalcoin:// to reversalcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("reversalcoin://"))
    {
        uri.replace(0, 12, "reversalcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ReversalCoin.lnk";
}
...
{
    return GetAutostartDir() / "reversalcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ReversalCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  reversalcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("ReversalCoin-Qt"));
